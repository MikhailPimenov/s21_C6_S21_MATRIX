...
void get_little_matrix(const matrix_t *a, int row_to_exclude,
                       int column_to_exclude, matrix_t *result) {
  assert(row_to_exclude < a->rows && column_to_exclude < a->columns &&
         "Row or column is out of range for submatrix!");
  ...
  for (int row = 0; row < a->rows; ++row) {
    ...
    for (int column = 0; column < a->columns; ++column) {
      if (column == column_to_exclude) continue;
...
int is_nan(double number) { return number != number; }
...
static double determinant(const matrix_t *a) {
  assert(a->rows == a->columns &&
         "Matrix should have same dimensions to calculate determinant!");

  if (a->rows == 1) return a->matrix[0][0];

  if (a->rows == 2)
    return determinant2x2(a->matrix[0][0], a->matrix[0][1], a->matrix[1][0],
                          a->matrix[1][1]);
  ...
  for (int column = 0; column < a->columns; ++column) {
    ...
    result +=
        (a->matrix[0][column] * sign(0, column) * determinant(&little_matrix));
  }
